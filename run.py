...
ALL_MODELS = sum((tuple(conf.keys()) for conf in (BERT_PRETRAINED_CONFIG_ARCHIVE_MAP,
                                                  XLM_ROBERTA_PRETRAINED_CONFIG_ARCHIVE_MAP)), ())
MODEL_CLASSES = {
    'mbert': (BertConfig, BertForQuestionAnswering, BertTokenizer),
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids': batch[0],
                      ...
                      'token_type_ids': None if args.model_type == 'xlm-r' else batch[2],
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {'input_ids': batch[0],
                      'attention_mask': batch[1],
                      'token_type_ids': None if args.model_type == 'xlm-r' else batch[2]  # XLM don't use segment_ids
                      }
...
def load_and_cache_examples(args, tokenizer, evaluate_mode=False, output_examples=False):
    if args.local_rank not in [-1, 0] and not evaluate_mode:
    ...
    if args.local_rank == 0 and not evaluate_mode:
...
def main():
    ...
    parser.add_argument("--train_file", default="", type=str, required=True,
                        help="Data json for training. E.g., train-v1.1.json")
    parser.add_argument("--predict_file", default="", type=str, required=True,
                        help="Data json for predictions. E.g., dev-v1.1.json or test-v1.1.json")
    ...
    if args.local_rank == 0:
        torch.distributed.barrier()  # Make sure only the first process in distributed training
