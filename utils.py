...
from utils_evaluate import make_qid_to_has_ans, get_raw_scores
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        tok_to_orig_index = []  # kiểm tra xem một token thuộc từ nào trong câu (do tokenize ra subwords)
        orig_to_tok_index = []
        all_doc_tokens = []
        # tokenize context
        ...
        # tìm token bắt đầu và kết thúc trong context do việc tokenize subwords
        # khiến start, end index ban đầu không còn chính xác
        tok_start_position = None
        tok_end_position = None
        # nếu câu hỏi không thể trả lời được thì gán cho -1
        ...
        if is_training and not example.is_impossible:
            ...
            # thực hiện improve span do subwords tokenize khiến các câu trả lời rõ ràng hơn
            (tok_start_position, tok_end_position) = _improve_answer_span(
                all_doc_tokens, tok_start_position, tok_end_position, tokenizer,
                example.orig_answer_text)
        # Trừ ra 3 token đặc biệt
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    def _strip_spaces(text):
        ...
        for (i_char, c) in enumerate(text):
            if c == " ":
                continue
            ns_to_s_map[len(ns_chars)] = i_char
            ns_chars.append(c)
